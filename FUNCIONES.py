...
def dar_alta_paciente(salida_p):
    ...
    while True:
        try:
            ...
            diastolica = int(input("Ingrese diastolica (MMHG): "))
        except ValueError:  
            continue  
    ...
    while True:
        try:
            ...
            continue
        if(temperatura<50 and temperatura>27):
            break
        else:
            print("El dato ingresado es invalido, vuelve a ingresar.")
            continue
    ...
    while True:
        try:
            ...
            continue
        if(colesterol>0 and colesterol<500):
            break
        else:
            print("El dato ingresado es invalido, vuelve a ingresar.")
            continue
...
def ecg_senyales(entrada_s):
    ...
    for dato in datos:
        ...
        min_senyales.append({"archivo": dato, "min": float(senyales.min())})
        prom_senyales.append({"archivo": dato, "promedio": sum(datos)/len(datos)})
            #uso funciones integradas
            #ahora tengo un diccionario con los valores maximos y minimos relacionado al key de archivo 
    return max_senyales, min_senyales, prom_senyales
